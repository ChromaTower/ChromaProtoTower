...
public class ControllerUI2 : MonoBehaviour {
...
[HideInInspector]	public List<Transform> childButtons; // The buttons that are children of the gameobject
	...
	void Update () {
		...
		if (OtherLists.Count > 1) {
			if (Input.GetAxis ("DPadX2") == 1 && dPadInputX == false) { // This produces the selecetion for the buttons using your controller
				dPadInputX = true;
		...
		if ((Input.GetAxis ("DPadY2") == -1 || Input.GetKeyDown(KeyCode.DownArrow))
		    && dPadInputY == false) { // This produces the selection for the buttons using your controller
			dPadInputY = true;
		...
		if ((Input.GetAxis ("DPadY2") == 1 || Input.GetKeyDown(KeyCode.UpArrow))
		&& dPadInputY == false) { // This produces the selecetion for the buttons using your controller
			dPadInputY = true;
